...
def bt_loss(f, p, d, h, R, Ct, zone, htg, hrg, pol, phi_t, phi_r, lam_t, lam_r, **kwargs):
   ...
   Other functions called from this function are in ./private/ subfolder.
     ...
     DN      -   穿过大气层最低1公里的平均无线电折射率衰减率（在此过程中为正数）（N单位/km）
     N0      -   海平面表面折射率仅由对流层散射模型用作对流层扩散机制的位置变异性的度量。
                 DN 和 N0的正确值由从适当的映射（N个单位）导出的路径中心值给出
...
def smooth_earth_heights(d, h, R, htg, hrg, ae, f):
    ...
    for ii in range(1, n):
        v2 = v2 + (d[ii] - d[ii - 1]) * (
                h[ii] * (2 * d[ii] + d[ii - 1]) + h[ii - 1] * (d[ii] + 2 * d[ii - 1]))  # Eq (86)
...
def dl_se(d, hte, hre, ap, f, omega):
    ...
    if d >= dlos:
        ...
        b = 2 * np.sqrt((m + 1.0) / (3.0 * m)) * np.cos(
            np.pi / 3 + 1.0 / 3.0 * np.arccos(3 * c / 2.0 * np.sqrt(3.0 * m / (m + 1.0) ** 3)))  # Eq (24c)
...
def dl_bull_att4(dtot, hte, hre, ap, f):
    ...
    if dtot < dlos:  # LoS
        ...
        b = 2 * np.sqrt((m + 1.0) / (3.0 * m)) * np.cos(
            np.pi / 3.0 + 1.0 / 3.0 * np.arcos(3.0 * c / 2.0 * np.sqrt(3.0 * m / ((m + 1.0) ** 3))))  # Eq (24c)
